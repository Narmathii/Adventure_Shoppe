...
GROUP BY `product_id` HAVING SUM(`prod_count`)<=10";
    ...
    public function products($segName, $segID)
    {
        ...
        // Fetch common products based on modalID
...
WHERE b.flag = 1 AND b.prod_id = ? ;";
        ...
        //     ON a.modal_id = b.modal_id 
        //     WHERE b.modal_id = $modalID  AND b.flag = 1

        //     UNION

        //     SELECT NULL AS modal,                                                                                                                                             
        //         b.* 
        //     FROM tbl_products AS b 
        //     WHERE b.modal_id = 0 AND b.flag = 1;")->getResultArray();
...
";
        ...
        $res['search_brand'] = array_values(array_map("unserialize", array_unique(array_map("serialize", $searchBrandArr))));
    ...
    public function shopbybrand($segName): string
    {
        ...
        $res['modal_list'] = $db->query(
            "SELECT  a.`modal_name`,a.`modal_img`, b.brand_name FROM `tbl_modal_master` AS a 
    ...
    public function accessories($segName, $subaccID)
    {
    ...
    public function ridingAccessories($segName, $sID)
    {
        ...
        $res['search_brand'] = $db->query($searchquery, [$subID])->getResultArray();
    ...
    public function loadmoreRidingDetails()
    {
        ...
        $res = $db->query("SELECT a.r_sub_menu , b.* FROM 
        tbl_riding_submenu  AS a INNER JOIN tbl_rproduct_list AS b 
        ON a.r_sub_id = b.r_sub_id
    ...
    public function loadmoreHDetails()
    {
        ...
        $res = $db->query("SELECT a.h_submenu , b.* FROM 
            tbl_helmet_submenu AS a INNER JOIN tbl_helmet_products AS b 
            ON a.h_submenu_id = b.h_submenu_id 
            WHERE a.h_submenu_id = $subID AND b.flag =1 LIMIT 12
             OFFSET $dataLimit ")->getResultArray();
    ...
    public function loadmoreCampDetails()
    {
        ...
        $res = $db->query("SELECT a.c_submenu, b.* FROM tbl_camping_submenu AS a INNER JOIN tbl_camping_products AS b ON a.c_submenu_id = b.c_submenu_id WHERE b.flag = 1 AND a.c_submenu_id =$subID
         LIMIT 12  OFFSET $dataLimit")->getResultArray();
    ...
    public function loadmoreAccessDetails()
    {
        ...
        $res = $db->query("SELECT a.* ,b.sub_access_name
            FROM tbl_accessories_list AS a INNER JOIN tbl_subaccess_master AS b
            ON a.sub_access_id = b.sub_access_id
            WHERE b.sub_access_id = $subID
            AND a.flag = 1 LIMIT 12
    ...
    public function touringAccessories($segName, $lugID)
    {
        $segment = strtolower(str_replace('-', ' ', $segName));
    ...
    public function helmetAccessories($segName, $hsubID)
    {
        $segment = strtolower(str_replace('-', ' ', $segName));
    ...
    public function Brands($segName, $segID)
    {
        ...
        $query = "SELECT *,offer_price AS prod_price,offer_type AS offer FROM tbl_products WHERE `flag` = 1 AND `search_brand` = $BrandID
                    UNION 
                    SELECT *,offer_price AS prod_price,offer_type AS offer  FROM tbl_accessories_list WHERE `flag` = 1 AND `search_brand` = $BrandID
                    UNION 
                    SELECT * ,offer_price AS prod_price,offer_type AS offer   FROM tbl_rproduct_list WHERE `flag` = 1 AND `search_brand` = $BrandID
                    UNION
                    SELECT *,offer_price AS prod_price,offer_type AS offer  FROM tbl_helmet_products WHERE `flag` = 1 AND `search_brand` = $BrandID
                    UNION 
                    SELECT *,offer_price AS prod_price,offer_type AS offer  FROM tbl_luggagee_products WHERE `flag` = 1 AND `search_brand` = $BrandID
                    UNION 
                    SELECT * ,offer_price AS prod_price,offer_type AS offer  FROM tbl_camping_products WHERE `flag` = 1 AND `search_brand` = $BrandID;";
    ...
    public function newArrivalViewall()
    {
        $db = \Config\Database::connect();
...
")->getResultArray();
    ...
    public function loadmoreOffer()
    {
        ...
        $res = $db->query("SELECT prod_id, access_id AS brand_id, sub_access_id AS modal_id, product_name, 
                billing_name, product_price, offer_price, offer_type, offer_details,
                arrival_status, stock_status, redirect_url, product_img, img_1, img_2, 
                img_3, img_4, img_5, img_6, img_7, img_8, img_9, img_10, prod_desc, hot_sale, tbl_name, 
                search_brand, weight, weight_units, quantity, specifications, flag, 2 AS order_col
    ...
    public function campingProducts($segname, $segmentID)
    {
