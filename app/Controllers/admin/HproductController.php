...
use App\Models\admin\HelmetProdModel;
...
class HproductController extends BaseController
{
    ...
    public function productList()
    {
        ...
        $res['menu'] = $db->query('SELECT `h_menu_id`,`h_menu` FROM `tbl_helmet_menu` WHERE  `flag` =1')->getResultArray();
        ...
        if ($session->get('login_sts') == "") {
            ...
            return view('admin/helmetProducts', $res);
        }
    ...
    public function insertproductList()
    {
        ...
        $modal = new HelmetProdModel;
        $configg = new ProdConfigModel;

        $db = \Config\Database::connect();

        $data = $this->request->getPost();
        $data['tbl_name'] = "tbl_helmet_products";
    ...
    public function getproductList()
    {
        ...
        $query =
            'SELECT a.h_menu, d.prod_id as configid, b.* , c.h_submenu , d.`config_id`,  d.`tbl_name`, d.`colour`, d.`size`, d.`soldout_status`
            ,d.`config_img1`,
            ...
            c.h_submenu_id = b.h_submenu_id
             ...
             WHERE b.flag = 1 ORDER BY h_menu ASC';
    ...
    public function updateproductList()
    {
        ...
        $tbl_name = "tbl_helmet_products";

        ini_set('memory_limit', '1024M');

        // product Images and image1 - image 10
        $query = "SELECT product_img,img_1,img_2,img_3,img_4,img_5,img_6,img_7,img_8,img_9,img_10 FROM `tbl_helmet_products` WHERE `flag` = 1 AND `prod_id` = ? ";
    ...
    public function deleteproductList()
    {
        ...
        $query = 'update`tbl_helmet_products` SET `flag` = 0 WHERE `prod_id` = ?';
    ...
    {
        $db = \Config\Database::connect();
        $h_menu_id = $this->request->getPost('h_menu_id');


        $query = 'SELECT `h_submenu_id`,`h_submenu` FROM tbl_helmet_submenu 
        WHERE `h_menu_id` = ? AND flag = 1';
        $res = $db->query($query, [$h_menu_id])->getResultArray();

        echo json_encode($res);
    }

    public function exportHelmetStock()
    {
        ini_set('memory_limit', '1024M');
