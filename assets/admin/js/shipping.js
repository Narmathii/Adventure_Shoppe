$(document).ready(function () {
  ...
  function getOrderList() {
    $.ajax({
      type: "POST",
      url: base_Url + "get-shipping-status",
      dataType: "json",
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (i = 0; i < sizee; i++) {
          ...
          <div class="mb-1 fs-14 fw-semibold order-prodname">
              <a href="javascript:void(0);">${viewOrder[i]["product_name"]}</a>

          </div>
          
          <div class="fs-14 fw-semibold size ${className}">
              <p>Size : ${size}</p>
          </div>
          <div class="fs-14 fw-semibold color ${classNamee}">
              <p>Color : ${color}</p>
          </div>
